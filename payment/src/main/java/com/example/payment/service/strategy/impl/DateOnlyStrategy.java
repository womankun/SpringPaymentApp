...
@Component
public class DateOnlyStrategy implements PaymentRecordStrategy {
    private final PaymentRecordService service;

    public DateOnlyStrategy(PaymentRecordService service) {
        this.service = service;
    }

    @Override
    public PaymentRecordResponse execute(PaymentRecordRequest request) {
        return service.selectRecordByDate(request.getStartDate(), request.getEndDate());
    }
}